...
export async function GET(req: NextRequest) {
  ...
  // Allow public access to all resource types for GET
...
export async function PUT(req: NextRequest) {
  ...
  const result = await collection.updateOne(
    { _id: new (require('mongodb').ObjectId)(id || _id) },
    { $set: rest }
  )
  if (result.matchedCount === 0) return NextResponse.json({ message: 'Not found' }, { status: 404 })
...
export async function DELETE(req: NextRequest) {
  ...
  const result = await collection.deleteOne({ _id: new (require('mongodb').ObjectId)(id || _id) })
